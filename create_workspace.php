...
<?php
  ...
  function createWorkspace() {
    global $wid, $pname, $date, $descr, $manager, $developer;
    global $nameErr, $descErr, $mngErr;
...
function getContent() { 
  global $pname, $nameErr, $descr, $descErr, $mngErr;
  echo '
    <script>
    function getuser() {
  ...
  <body>
    <div class="admin">
      <form action="/action/createWorkspace" method="post">
        Project Name <input type="text" name="pname" value="' . $pname . '"><span class="error">' . $nameErr . '</span><br>
        Description: <br><br><textarea rows="5" cols="50" name="description" value=" '. $descr . '"></textarea><span class="error">' . $descErr . '</span><br><br><br>
        <input type="button" onclick="getuser()" value ="Add managers and developers"><br><br>
        <div id ="txtHint" class="white"><span class="error">' . $mngErr . '</span><br></div>
        <br><br>
